...
**🧑‍🤝‍🧑 1. Four people, written down as data (3 minutes).** Dana Okonkwo the teacher, Malik Ferrer the student, Ana Whitfield the advisor, and Bea Njoroge the family member, each with goals, fears, decision rights, the grain they think in, what they cannot change, and their access reality. Read the decision rights carefully: they are the field most dashboards skip, and a metric handed to someone with no authority to act on it is not information, it is pressure. Two of these four can change something about the course. Two can only change how they talk to a person.

Read the goals as carefully as the decision rights, because a persona card is where a deficit assumption hides most comfortably. Ana's first goal is to know what an advisee is already doing well so that a call is not only ever about trouble, and one of her stated fears is deficit language landing in an advising note that follows a student for years. Bea wants to understand what her student is working on well enough to be useful when asked, and one of her fears is a panel that only ever tells her what is going wrong. Neither card treats the student as a problem to be detected, and if yours does when you write a fifth persona, that is the thing to notice.

**📊 2. The metric menu (3 minutes).** Eleven metrics computed from the real data, one number per student, so that putting one on a screen means putting an actual distribution on a screen. Each carries three annotations that matter more than its formula: its direction (eight of the eleven are marked *contested*), what a tired reader will mistake it for, and who can act on it, scored per persona as 0, 1, or 2. One metric, gaze at peers, scores 0 for everybody. It is on the menu because a camera can produce it, which is the whole of Carvalho and colleagues' argument in a single row.

The *mistaken for* column is doing the framing work. It is where the deficit reading is named as the error rather than smuggled in as the label: submission timing is mistaken for "procrastination, and then a character flaw", share of the floor for "dominance, or not pulling your weight", and starting while someone else is speaking for "rudeness, and then a judgment about the person". Each label describes what was measured, and each annotation names the leap you are not entitled to make. Note also that `last_minute_rate` counts submissions arriving after the deadline as well as those in the final six hours (33 and 175 of the 960 respectively), so its label names both rather than only the flattering half.
...
**📊 The sketch (3 minutes).** Two plotly panels, because nearly every learning analytics view is secretly one of these two shapes: the class distribution of your first metric with your focus student marked, and your focus student's class percentile on each metric you chose. Read the two warnings attached to the second panel. Converting everything to percentiles makes mixed units comparable and also guarantees that somebody is at the bottom, even in a class where everyone is fine. And a percentile bar reads as "longer is better" whatever it is measuring, so on `low_quiz_count` and `last_minute_rate` a long bar means the opposite of what the eye assumes. The colour encodes direction, which is the fix every dashboard reaches for and which only works on a reader who reads the legend. The default focus student, S022, sits near the bottom of the class on speaking share, near the top on ideas contributed in chat, and well above the class average on quizzes. The prompt asks which of your four metrics would have found this student, and which would have misfiled them.

**📊 4. Whose story does each metric change (4 minutes).** For every metric on the menu, the standardized difference (Cohen's *d*) between a comparison group and everyone else, **with a 95 percent interval on every row**, because 24 students against 96 pins a *d* down only to about plus or minus 0.45 and eleven comparisons are being run at once.

With the default group, the 24 multilingual students, the pattern is not "these students participate less." It is that **the channel decides the answer**. Five of the eleven intervals clear zero: speaking time -0.59, share of the floor -0.58, speaking turns -0.58 and interruptions -0.47 on one side, ideas contributed in chat +0.52 on the other. Gaze at peers (+0.42) and document edits (+0.41) point the same way and their intervals do not clear zero, so the honest sentence for those two is that this file cannot tell. The grade metrics sit near zero throughout. Even on the reduced list, a view built on speaking time and a view built on chat ideas would generate two opposite lists of students to worry about from the same 120 people.
...
**🕰️ 6. Retroactive design audit (4 minutes).** Every artifact this course built from week 3 to week 10, the design decision each one quietly made, and a score from 0 to 3 for how much voice each persona should have had: none, informed, consulted, veto. The dictionary comes pre-filled with the instructor's first guess and you are expected to argue with it. In the default scoring the student column totals 23 out of 24 and no student was in the room for any of it.

Read the wording of the artifact descriptions as part of the audit. Each one is in the active voice, because the point is that somebody chose, and where a week used one of the field's deficit-coded terms the description quotes it rather than repeats it: week 3 *chose to call* low recorded activity "risk", week 7 *called* some patterns "hint spam" on a threshold we picked, week 8 *called* four students "connectors" and treated submitting close to the deadline as a risk factor. Which words an artifact chose is one of the design decisions on the table, and every one of those words arrived without the person it describes in the room.

One nuance to catch: week 10 is FractionQuest, played by 12-year-olds, and the same four persona labels carry very different weight when the learner is a child. The pre-filled parent score for that row is deliberately low so there is something to argue with, and the appendix works through the move most people make.
...
- **Bang and Vossoughi (2016)**, *Participatory design research and educational justice: Studying learning and relations within social change making*: participatory design is about studying and changing relations, not about collecting stakeholder preferences and then proceeding as planned. The decision-rights heatmap is a picture of a relation. Every studio metric is a 2 for the teacher and a 0 for both the advisor and the family member; the student scores 1 on most of them, which is to say the person being measured can act only after a conversation with the person doing the measuring. That asymmetry is the relation, and it is worth naming before you propose another one.
...
3. **Change the comparison group, and be careful what you conclude.** Re-run section 4 with `COMPARE_GROUP` set to `'works_15plus'`, then to `'first_gen'`. The multilingual comparison produces a clean modality story with five intervals clearing zero. For the other two groups **every interval crosses zero, on all eleven metrics**. The tempting sentence is "the menu is fairer to those groups"; it is not supported, because an absence of a detectable difference at these group sizes is not evidence of an absence of a difference. Write the correct sentence instead, work out roughly how many students you would need to detect a *d* of 0.3, and then say which group your Rough Draft's proposed system would be least able to check itself against.
