...
**📊 2. Complete regulation loops (7 minutes).** Profiles count actions. Regulation is about order, because setting a goal after you finish is not planning. The notebook states a rule you can hold in your head: a session contains a **complete regulation loop** when a `set_goal` occurs, then an `attempt` at any point after it, then a `review_feedback` or a `reflect` at any point after that. The three actions do not have to be adjacent. Write the definition down. Week 11's retroactive design audit puts this lab back on the table and asks who should have had a say in labelling a student's tutor use as good strategy or as hint spam. A small state machine applies the rule to all 844 sessions: 410 of them contain a loop, which is 48.6 percent, and 406 of the failures are sessions where no goal was ever set. Loop rate then goes up against **quiz growth**, the slope of a line through each student's eight quiz scores, and comes back at `r = 0.58`. Growth rather than average score, because average score mostly tells you who arrived well prepared. ✏️ **Your turn 2** tightens the definition so that only `reflect` closes a loop: prevalence falls to 30.6 percent and `r` falls to 0.52. A measurement decision just changed a finding.
