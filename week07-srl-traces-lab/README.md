...
**📊 1. What a trace actually looks like (4 minutes).** Seven action types exist in this tutor: `set_goal`, `view_plan`, `attempt`, `request_hint`, `check_answer`, `review_feedback`, and `reflect`. Look at the raw rows before you compute anything, and notice what is missing: there is no column for whether the student was regulating their learning. The class-wide counts are already an argument. There are 9,866 attempts and 9,273 hint requests, but only 438 goals and 376 reflections. Then one session is printed in order, as a timeline with the seconds that passed between actions, and it is worth slowing down for: S062 opens with `set_goal` and `view_plan`, thinks for 86 seconds after checking an answer, and then requests six hints on a single item in twenty seconds, with gaps of 2 to 5 seconds. Every argument the rest of the session makes is visible in that one screen. Section 1.1 turns the log into per-student profiles and shows the spread: the median student spends about 19 percent of their tutor actions on hints, and somebody spends 67 percent. ✏️ **Your turn 1** puts any single student against the class median.

**📊 2. Complete regulation loops (7 minutes).** Profiles count actions. Regulation is about order, because setting a goal after you finish is not planning. The notebook states a rule you can hold in your head: a session contains a **complete regulation loop** when a `set_goal` occurs, then an `attempt` at any point after it, then a `review_feedback` or a `reflect` at any point after that. The three actions do not have to be adjacent. Write the definition down, because Week 11 refers back to this exact wording when we co-design an SRL dashboard. A small state machine applies the rule to all 844 sessions: 410 of them contain a loop, which is 48.6 percent, and 406 of the failures are sessions where no goal was ever set. Loop rate then goes up against **quiz growth**, the slope of a line through each student's eight quiz scores, and comes back at `r = 0.58`. Growth rather than average score, because average score mostly tells you who arrived well prepared. ✏️ **Your turn 2** tightens the definition so that only `reflect` closes a loop: prevalence falls to 30.6 percent and `r` falls to 0.52. A measurement decision just changed a finding.

**📊 3. Hint spam, and why counting will not find it (7 minutes).** Here is a claim worth being suspicious of: students who ask for many hints are gaming the system. Asking for a lot of hints can mean a student is stuck and seeking help, which is a regulation strategy rather than a failure of one. The counts look identical, and only the clock can tell them apart. So the notebook computes the seconds between each action and the one before it: half of all hint requests arrive within 10 seconds of the previous action. A rule follows (three or more consecutive hints, each within 10 seconds, at least three times across the term) and flags 18 of the 120 students. The pair to watch is S017 against S072. S017 asks for 14.2 hints a session and waits a median of 21 seconds before each one, and is not flagged. S072 asks for 21.0 and waits 4 seconds, firing six hints in nineteen seconds. Nobody reads a hint in three seconds. ✏️ **Your turn 3** moves both thresholds, and in this dataset the flag barely moves, which the notebook is careful to call luck rather than method.

**📊 4. Putting the two behaviors next to learning (4 minutes).** Sort every student into frequent loopers, hint spammers, and everyone else, and compare quiz growth. The loopers sit clearly above zero at +0.66 and both other groups sit below it. Then look again, because **hint spammers and everyone else have almost the same mean growth**, -0.55 against -0.56. The gap that makes hint spamming look damaging is mostly a gap between spammers and loopers, not between spammers and ordinary students. The interpretation prompt here has teeth, and it asks what you would be entitled to do on Monday morning if somebody handed you this chart and a list of 18 names.

**📊 5. What follows what: the transition heatmap (6 minutes).** Loop rate compresses a whole session to one bit. Bigrams keep more of the sequence: count every neighbouring pair of actions, put the earlier action in the rows and the later one in the columns, and divide each row by its total. Class-wide, an attempt is followed by a check 72 percent of the time and a hint request is followed by another hint request 42 percent of the time. Section 5.1 then computes the identical grid twice, once for loopers and once for spammers, with a difference panel. The `request_hint` to `request_hint` cell reads 0 percent for loopers and 77 percent for spammers, which is about as stark as behavioral data gets and is also **circular**, because the groups were defined using hint timing in the first place. Naming the circularity is the point, and the prompt asks you to find two contrasts in the difference panel that are not circular. ✏️ **Your turn 4** splits by outcome instead of by profile, which is the non-circular version of the same question.
...
- **Winne (2022)**, *Learning analytics for self-regulated learning*: the argument the entire lab is built on, that self-regulated learning is not directly observable and traces are the footprints it leaves. Our loop rate is a coarse stand-in for the recursive phase structure Winne describes, with `set_goal` standing in for planning, `attempt` for enacting, and `review_feedback` or `reflect` for evaluating. The reflection asks the honest question: what does your loop rate get most wrong about a real student, and would more trace data fix it or is the problem of a different kind?
...
2. **Make the loop a real cycle rather than a single pass.** The current rule asks whether a session contains at least one loop. Count how many complete loops each session contains, using a matcher that resets to stage 0 after each closure, and use loops per hour of tutor time as the measure instead of a binary. Re-run the correlation with quiz growth. If a count beats a yes-or-no, that is an argument about measurement worth putting in a paper.
...
5. **Sensitivity as a deliverable, not a footnote.** The appendix sweeps the hint-spam thresholds and finds the flag remarkably stable, then warns you not to generalize that stability. Do the same sweep for the loop definition: vary which actions open and close a loop, and plot both prevalence and the correlation with growth across every combination. The notebook already reports that prevalence swings by tens of percentage points while the correlation barely moves. Explain why, in two sentences, and you will have understood something most published papers do not report.
...
**My loop rate is not 48.6 percent.** If you changed the closers in Your turn 2, that is expected and it is exactly what the exercise is for: `reflect` alone gives 30.6 percent. If you did not, restart and run all. The data generator is seeded, so a clean run reproduces the same numbers every time.

**Nothing changed when I moved the hint-spam thresholds.** That is the real result in this dataset, and the notebook says so: the planted behavior is extreme enough that the same 18 students get flagged across almost every combination. The lesson is not that thresholds do not matter. It is that you cannot know whether yours matters until you sweep it, which is what the appendix does.
