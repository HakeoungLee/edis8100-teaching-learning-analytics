...
1. **Weight the assessments properly.** Section 5 takes an unweighted mean, and the notebook says so and says why it is wrong. Use the `weight` column in `assessments.csv` to compute a weighted coursework score instead, then re-run the Section 7 comparison. Does the log transformation still nearly double the Pearson correlation, does the Spearman correlation move at all, and does the ordering of the deciles change?
