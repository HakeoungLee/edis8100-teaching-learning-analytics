...
This is the first notebook of EDIS 8100, posted for you to work through on your own. It opens in
Google Colab, runs on a real published dataset, makes three figures, looks at a pattern that the
summary numbers do not show, and ends with a question the data cannot answer.

Nothing in it asks you to write code from scratch, this week or in any later week. You run cells,
read what comes out, and change one clearly marked value to see two summary numbers change.
...
| **Notebook** | `week01_hello_learning_analytics.ipynb`. Posted for independent work. The core path takes about 55 to 60 minutes. |
...
| **Deliverable** | None. Nothing from this notebook goes to Canvas. |
| **Due** | Nothing. Nothing from this notebook is submitted. |
...
The setup cell downloads `student-mat.csv` and prints `Loaded 395 students and 33 columns.`

| | Sections | What it is for |
|---|---|---|
| **CORE, about 55 to 60 minutes** | Setup; Where this data came from; 1. Meet the table; 2. A first look at a group difference; 3. The shape of one variable; 4. Looking at the zeros; Your turn; 5. Reflection; Before you leave today | Provenance, what one row and one column record, a group difference read with its group sizes, the shape of the final grade, the zero records, and one named decision that changes the mean and the pass rate |
| **IF TIME** | Going further: A group comparison, read carefully; Appendix: notes on the Your turn cell | A difference between two recorded means with an interval beside it, before and after the zero records are set aside, and the methods sentence that names the choice |
| **EXPLORE LATER** | Other things to try | The zeros in `absences`, the first period grade, other columns, and the mother's-education chart without the zero records |

## The figures the notebook makes
...
1. **Mean final grade by mother's education, twice.** The first version rises across bands 1 to 4
   ...
   band 0 is three students with grades of 9, 15 and 15. The group size belongs next to the mean.
   Bands 1 to 4 do rise, from 8.68 to 11.76 on 59, 103, 99 and 131 students. A difference between
   recorded groups is, in the first instance, a difference in recorded grades between recorded
   groups. Whether it is evidence about the students' circumstances, about how the school recorded
   outcomes, or about the students themselves is a separate question, and one this file cannot
   settle.
2. **A histogram of all 395 final grades.** Roughly symmetric around 11, with a tall spike at
   exactly 0 and nothing at 1, 2 or 3. That is an unusual shape, and Section 4 looks at the zeros.

## The pattern we look at

All 38 students with a final grade of exactly 0 have zero recorded absences, none of the 280
students with any recorded absence has a final grade of 0, and every one of the 38 has a first
period grade above zero. For comparison, 115 students in the file have zero recorded absences in
total, so a zero absence record does not by itself go with a zero grade.
...
Treating those 38 as missing changes the mean from 10.42 to 11.52 and the pass rate from 67.1
percent to 74.2 percent. Both numbers are defensible, as long as the choice is named. The group means
by mother's education were computed with the zeros counted as grades, so they depend on the same
choice.
...
time for. Please keep it for the course project.
...
**My numbers differ from a classmate's**
One of you has changed the Your turn value. Comparing the two settings and explaining the difference
to each other is a useful thing to do.

**An error appeared**
The **last line** of the error usually names the real problem. If it does not, **Runtime > Restart
session and run all**.
...
There is **nothing to submit this week**, so there is nothing to document. Beginning with Mini
Project 1 in Week 4, every mini project and every course project milestone asks for an **AI
Reflection** submission on Canvas, with two parts in two places:
...
- **Wise (2019)** distinguishes activity, artifact, association, and contextual data, and notes that
  ...
  a justified link between what is observed and the construct of interest, applies to the mother's
  education column. Her framing of data-informed decision-making is what the pass rate question is
  about: the number contributes to a decision rather than dictating it.
- **Lang and colleagues (2022)** present learning analytics as a concern, an opportunity, a field of
  inquiry, and a community, and describe it as dealing with learners *and their contexts*. Which
  columns in this file are context, and which contexts have no column at all?
...
This file holds records for real teenagers in two Portuguese secondary schools, assembled from
school reports and a student questionnaire in the 2005 to 2006 school year. They answered questions
about their households and their lives. The file holds no names or other identifiers, and it is
