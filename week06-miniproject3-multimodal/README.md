...
**📊 4. Who flips? The whole class at once (8 minutes).** Two students and one group are anecdotes. Now plot all 120 by two percentiles, speaking time against idea units, with the region that should not exist shaded: quiet on audio, top third on ideas. Sixteen students live there. Multilingual students are 20 percent of this class and 56 percent of that group, which is 2.8 times their base rate. The notebook then does the thing a dashboard would not: it puts an interval on a proportion computed from sixteen people. The concentration is very unlikely to be chance (exact binomial *p* = 0.0015) and its size is known only to within roughly 33 to 77 percent. Both sentences are true and they say different things. This is the finding the memo has to deal with, and the question the notebook asks is whether it is a story about those students or a story about the microphone.

**📊 5. Disaggregating: the same class, seven measures, two stories (8 minutes).** Recompute every modality separately for multilingual and non-multilingual students, standardized as Cohen's *d* so that seconds and counts can sit in the same chart, and report an interval on every one. Speaking time comes back at *d* = -0.59 [-1.04, -0.14] and turns at -0.58. Idea units come back at +0.52 [+0.07, +0.97] and doc edits at +0.41 [-0.04, +0.86]. The audio and video streams point one way, the digital streams point the other, and a single-modality report would have confidently published half of that. Read the intervals before the bars: with 24 multilingual students, three of the four positive differences have an interval that touches zero, so the claim this class supports is the **sign reversal across sensors**, not the size of any one bar. ✏️ **Your turn 3** switches the grouping column to `first_gen`, where every interval contains zero. The honest sentence there is "this file cannot show a first-generation difference", which is not the same sentence as "there is no difference".

**📊 6. Measuring balance: the Gini coefficient, carefully (12 minutes).** The unit of analysis changes here, from the student to the group in one session. The section walks the intuition (line five people up from least talk to most, and watch how fast the accumulated share climbs), draws Lorenz curves for four invented groups so that a number like 0.45 acquires a feel, and states the formula. It also states the detail most dashboards get wrong: **in a group of five the maximum possible Gini is 0.8**, not 1.0. It then adds two preconditions most treatments skip: Gini needs a ratio scale with a real zero (which is why a Gini on `gaze_peer_pct`, a percentage that never leaves the range 17 to 61, is arithmetic rather than inequality), and the population form used here, dividing by *n*, is one of two conventions in circulation, so say which one you used and never compare across different group sizes. Then it computes the number for all 192 sessions. The mean is 0.348, the most balanced session is 0.070, and the most dominated is 0.681, a session in which one student took 80 percent of the floor and one took none.

**📊 7. Does balanced talk go with a better group product? (10 minutes).** Two ways of asking, because one way is never enough. Across 192 sessions the Pearson correlation between talk Gini and artifact score is -0.33 (Spearman -0.31); aggregated to 24 groups it is -0.52 (Spearman -0.51), 95% CI -0.76 to -0.15. The two lines have to be read differently and the notebook says why: the 192 sessions are 8 repeated measurements on each of 24 groups, so they are not 192 independent observations and the session-level number cannot carry a 192-based claim to precision. The most balanced third of sessions average 7.05 on the rubric and the most dominated third average 6.09, a difference of 0.96 points and a Cohen's *d* of +0.69, which is the same correlation restated in readable units rather than a second piece of evidence. Then read the warning underneath, which names two traps stacked on each other: this association is in the file because the generator put it there, and even at face value the scatter is a cloud whose interval spans a factor of five. Somebody is about to propose an alert that fires at Gini 0.5. ✏️ **Your turn 4** runs the same analysis on a different modality, and one of the columns is a trap the cell warns you about.

**✏️ 8. Stretch: build a participation index and defend it (15 minutes, optional).** Everything above this is the core path. Here you combine modalities into a weighted composite and re-rank the class. With voice alone, 11 of the 24 multilingual students land in the flagged bottom quarter, which is 46 percent of them. With an equal-weight composite of voice, ideas, and document work, 5 do, which is 21 percent. The section is explicit about what a composite is: **a choice about what the course will count, not a correction to a broken measure.** Nothing about the microphone improves when you average it with the chat log, anyone contributing in a way no sensor watched is still invisible, and thirty students sit below the line under either index. The composite changed who is standing in the bottom quarter, not whether a bottom quarter gets a label.
...
2. **Group composition, not group balance.** In this file, groups with more evenly shared talk have better-rated artifacts. Test the rival explanation. Compute, per group, the number of multilingual members and the mean prior GPA from `students.csv`, and regress artifact score on Gini plus those two. Does the Gini coefficient still carry its weight, or was it standing in for who happened to be in the room? Work at the group level, 24 rows, not the session level, because the 192 sessions are repeated measures on those same 24 groups.
...
**Cohen's *d* came out negative and I expected positive.** The sign is a direction, not a verdict. In Section 5 a positive *d* means that channel recorded more for multilingual students than for everybody else. Read the column header before you read the number, say the direction out loud in the memo rather than reporting a magnitude on its own, and quote the interval next to it.
