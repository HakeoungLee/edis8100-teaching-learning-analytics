...
Each notebook carries its own copy of the data generator, so it builds exactly the CSVs it needs inside your Colab runtime. You never have to clone this repository, never have to upload a file, and can never end up analyzing the wrong version of a dataset.

## 🌍 The data universe

Everything you analyze this semester comes from two invented places.

**EDUC 1010: Learning How to Learn** is a blended undergraduate course at the fictional Blue Ridge University, used in weeks 1 through 9 and week 11. One hundred and twenty students, eight instructional weeks running from 9/7 to 11/1, a Canvas-style LMS, a threaded discussion forum with one topic per week, twenty-four studio groups of five meeting Thursday afternoons, and an adaptive practice tutor. Between them they produce a roster, a clickstream, a gradebook, forum posts, studio chat, multimodal studio observations, group artifact scores, and self-regulated learning traces.

**FractionQuest** is a middle school fractions game, used in week 10. Two hundred players in grades 6 and 7, eight levels, a pre-test and a post-test out of 20, attempt-by-attempt telemetry, and in-game emotion self-reports.

Neither place exists. Every student, every click, every forum sentence, and every emotion ping was generated by `data/generate_all_data.py` with numpy seed 8100.

### The ethics note, which is not boilerplate

Learning analytics runs on data about people who usually did not get to weigh in on being measured. Practicing on real institutional data means practicing on real students, and a course assignment is a poor reason to touch someone's records. So we built a world instead.

**The data are synthetic precisely so that we can practice the judgment without surveilling anyone.** The ask in return is simple: **treat this data as if it were real.** Ask who could be harmed by a claim before you make it. Notice when a metric flattens a person. Notice when your model is confidently wrong about a group. That stance is not a warm-up for the course, it is one of the course's arguments.

You will also find that the data pushes back. Several patterns in here are built to punish a fast, careless reading, and finding that out in week 3 is much better than finding it out in a dissertation.

Full documentation, including the row-by-row data dictionary for all eleven files, is in [`data/README.md`](data/README.md).
