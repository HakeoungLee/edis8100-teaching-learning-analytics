...
| `README.md` | This file. |
| `data/` | Not used. It holds leftovers from the synthetic version of this lab and is ignored by git. Everything the notebook needs is downloaded from `github.com/HakeoungLee/edis8100-datasets`. |
...
*Chance.* Twenty thousand simulated draws per group, under equal typing propensities, give each group its own floor, and the floor is not zero and not constant: **0.053 for Group 5 with 260 messages, 0.098 for Group 8 with 106 messages and five members.** Six of the eight groups are more uneven than 95 percent of their own simulations. **Groups 1 and 6 are not distinguishable from chance at all.** And the two rankings disagree at the top: the most uneven group by raw Gini is Group 8, the most uneven above its own floor is Group 5.
