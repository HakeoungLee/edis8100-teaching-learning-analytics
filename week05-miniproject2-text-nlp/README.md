...
**📊 Section 8 (stretch) and ✏️ Your turn 5.** Try another group and another move. Evidence spans by writers classified as economically disadvantaged: 15.1 percent Effective against 26.6. Then try `gender`, where the spread is 4.6 points and points the other way, which is the point of the exercise: the gaps are not all the same size, and the largest fall on the classifications that track schooling conditions.
